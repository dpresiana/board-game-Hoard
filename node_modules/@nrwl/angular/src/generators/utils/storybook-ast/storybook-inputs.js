...
function getInputPropertyDeclarations(tree, path) {
    ...
    return decorators
        .filter((decorator) => (0, typescript_1.findNodes)(decorator, tsModule.SyntaxKind.Identifier).some((node) => node.getText() === 'Input'))
...
function getComponentProps(tree, componentPath, getArgsDefaultValueFn = getArgsDefaultValue, useDecoratorName = true) {
    ...
    const props = getInputPropertyDeclarations(tree, componentPath).map((node) => {
        const decoratorContent = (0, typescript_1.findNodes)((0, typescript_1.findNodes)(node, tsModule.SyntaxKind.Decorator).find((n) => n.getText().startsWith('@Input')), tsModule.SyntaxKind.StringLiteral);
